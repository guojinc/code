...
def random_change2(current_solution_copy):
...
def random_change(current_solution, D, I):
    ...
    for l in len(randemployee_1):
        for g in len(randemployee_2):
            ...
            if discontentment(D, current_solution, I) < best_discontentment:
                best_discontentment = discontentment(D, current_solution, I)
                best_solution = current_solution.copy()
...
def simulated_annealing(D, initial_solution,I ,initial_temperature=1, alpha=0.8,max_iter=10):
    ...
    while T > Tk and i < max_iter:
        ...
        # 随机选择一个任务并交换其执行:者
        # new_solution, new_discontentment = random_change2(current_solution, D, I)
...
if __name__ == '__main__':
    for i in range(10):
        ...
        # 现在你可以安全地写入文件了
        pd.DataFrame(best_solution).to_csv(os.path.join(directory, 'data2.csv'), index=False)
